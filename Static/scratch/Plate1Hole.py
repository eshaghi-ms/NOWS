...
x_coords = np.arange(x_min, x_max + mesh_size, mesh_size)
y_coords = np.arange(y_min, y_max + mesh_size, mesh_size)
nx = len(x_coords)
ny = len(y_coords)
...
mask_nodes_flat = dist >= radius  # Outside the hole: True
mask_nodes = mask_nodes_flat.reshape((ny, nx))

# Only include a cell if all four corner nodes are valid (outside the hole).
cells = []
for j in range(ny - 1):
    for i in range(nx - 1):
        if mask_nodes[j, i] and mask_nodes[j, i+1] and mask_nodes[j+1, i+1] and mask_nodes[j+1, i]:
            n1 = i      + j    * nx
            n2 = (i+1)  + j    * nx
            n3 = (i+1)  + (j+1)* nx
            n4 = i      + (j+1)* nx
...
cells_new = []
for cell in cells:
    cells_new.append([node_map[n] for n in cell])
cells = np.array(cells_new)
...
def element_stiffness_quad(xe, ye, D):
    """
    Compute the 8x8 stiffness matrix for a 4-node bilinear quadrilateral
    element using 2x2 Gauss quadrature.

    xe, ye : arrays of the x- and y-coordinates of the 4 nodes (length 4)
    D      : elasticity matrix (3x3)
    """
    ke = np.zeros((8, 8))
    # 2-point Gauss quadrature in [-1,1]
    ...
    for xi in gauss_pts:
        for eta in gauss_pts:
            ...
            # Compute derivatives w.r.t. x and y.
            dN_dx = invJ[0, 0] * dN_dxi + invJ[0, 1] * dN_deta
            dN_dy = invJ[1, 0] * dN_dxi + invJ[1, 1] * dN_deta
            # Build the B matrix (3 x 8).
            B = np.zeros((3, 8))
            for i in range(4):
                B[0, 2 * i] = dN_dx[i]
                B[1, 2 * i + 1] = dN_dy[i]
                B[2, 2 * i] = dN_dy[i]
                B[2, 2 * i + 1] = dN_dx[i]
            # Contribution from this Gauss point.
            ke += B.T @ D @ B * detJ
    return ke
...
for cell in cells:
    xe = points[cell, 0]
    ye = points[cell, 1]
    ke = element_stiffness_quad(xe, ye, D)
...
# (a) Neumann BC: Apply horizontal traction of 1 on the right boundary (x ~ 5)
right_nodes = np.where(np.abs(points[:, 0] - 5) < tol)[0]
# Sort right side nodes by their y-coordinate.
right_nodes = right_nodes[np.argsort(points[right_nodes, 1])]
for i in range(len(right_nodes) - 1):
    n1 = right_nodes[i]
    n2 = right_nodes[i+1]
    edge_length = np.linalg.norm(points[n1] - points[n2])
    # Apply traction in the x-direction (force component f[2*n]).
    f[2*n1] += 1 * edge_length / 2
    f[2*n2] += 1 * edge_length / 2

# (b) Dirichlet BC: Fix the left boundary completely (uₓ = 0 and u_y = 0 for nodes with x ~ 0).
left_nodes = np.where(np.abs(points[:, 0] - 0) < tol)[0]
...
# =============================================================================
# Timing: Step 8 - Save & Plot Field Variables
# =============================================================================
t8 = time.perf_counter()

scipy.io.savemat('elast2d_plate_hole_quad.mat', {
    'node_coords': points,
    'ux_ref': ux,
    'uy_ref': uy,
    'sigma_xx_ref': sigma_xx,
    'sigma_yy_ref': sigma_yy,
    'sigma_xy_ref': sigma_xy
})
