...
def plot_loss_trend(train_losses, test_losses, folder):
    # folder = 'Practical/' + folder
    fig_font = "DejaVu Serif"
    plt.rcParams["font.family"] = fig_font
...
def plot_PlateArbitVoid(dataset, index, data_type, model, model_params, model_data, field_names, folder):
    ...
    if model_data["normalized"]:
        ...
        x_data = np.where(np.array(x_data) < 0.5, False, True)
        y_data = dataset.normalizer_y.decode(y_data)
