...
"""
import numpy as np
from .bernstein import bezier_extraction
...
class IGAMesh2D:
    ...
    def __init__(self, patch):
        ...
        # store lengths and knot vectors for external use
        self.len_u = int(len_u)
        self.len_v = int(len_v)
        self.n = (self.len_u, self.len_v)
        self.knot_u = np.array(knot_u, dtype=float)
        self.knot_v = np.array(knot_v, dtype=float)
    ...
    def _get_boundary_indices(self):
        ...
        bnd_index : dict containing the "down", "up", "left", "right" boundary
                    indices
        """
        bnd_index = {"down": list(range(self.deg[0] + 1)), "right": list(range(self.deg[0], np.prod(self.deg + 1),
                                                                               self.deg[0] + 1)),
                     "up": list(range((self.deg[0] + 1) * self.deg[1],
                                      np.prod(self.deg + 1))),
                     "left": list(range(0, 1 + (self.deg[0] + 1) * self.deg[1],
                                        self.deg[0] + 1))}
        return bnd_index
    ...
    def setNodes(self, patch, numElemU, numElemV, numGauss):
        self.nodes = patch.getQuadIntPts(numElemU, numElemV, numGauss, output_nodes=True)
