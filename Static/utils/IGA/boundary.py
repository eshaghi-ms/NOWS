...
class boundary2D:
    ...
    patch_index : (int) index of the patch in a multi patch mesh
    ...
    for i in range(len(bound_cond)):
        ...
        if bound_cond[i].type == "Dirichlet":
            ...
            if side == "down" or side == "up":
                quadRuleSide = quad_rule[0]
            else:  # elif side == "left" or side == "right":
                quadRuleSide = quad_rule[1]
            rhs = applyNeumannScalar2D(mesh_list[patch_index], rhs, side, quadRuleSide, op_val)
...
def applyNeumannScalar2D(mesh, rhs, side, quad_rule_side, op_val):
    """
    Applies the Neumann boundary conditions on the given patch and side for
    ...
    quad_rule_side : (dict) Gauss points and weights in the reference
                interval [-1,1]
...
def applyRobinScalar2D(mesh, lhs, rhs, side, quad_rule_side, alpha, op_val):
    """
    Applies the Robin boundary conditions on the given patch and side for
    scalar problems and updates the rhs vector
...
def applyBCElast2D(mesh_list, bound_cond, lhs, rhs, quad_rule, return_Dirichlet_bcdof=False):
    """
    ...
    return_Dirichlet_bcdof: (boolean)
    ...
    for i in range(len(bound_cond)):
        ...
        if bound_cond[i].type == "Dirichlet":
            # check x-direction
            if bound_cond[i].op_value(eval_pt[0], eval_pt[1])[0] is not None:
                bcdof += [2 * j for j in mesh_list[patch_index].bcdof_global[side]]
                Dirichlet_bcdof += [2 * j for j in mesh_list[patch_index].bcdof_global[side]]
            # check y-direction
            if bound_cond[i].op_value(eval_pt[0], eval_pt[1])[1] is not None:
                bcdof += [2 * j + 1 for j in mesh_list[patch_index].bcdof_global[side]]
                Dirichlet_bcdof += [2 * j + 1 for j in mesh_list[patch_index].bcdof_global[side]]
        elif bound_cond[i].type == "Neumann":
            if side == "down" or side == "up":
                quad_rule_side = quad_rule[0]
            else:  # elif side == "left" or side == "right":
                quad_rule_side = quad_rule[1]
            rhs = applyNeumannElast2D(mesh_list[patch_index], rhs, side, quad_rule_side, bound_cond[i].op_value)
    ...
    if return_Dirichlet_bcdof:
        return lhs, rhs, Dirichlet_bcdof
    else:
        return lhs, rhs
