import os, jax, time, optax, pickle
import numpy as np
...
def model_evaluation(model, model_data, model_params, loss_fn, test_loader, normalizers=None, output=False, mask=None):
    ...
    test_losses_argmin = jnp.argmin(test_losses_array)
    ...
    print("Index of minimum error is", test_losses_argmin.item())
    if output:
        return test_losses_avg


def train_fno(model, train_loader, test_loader, loss_fn, get_params, update_fun, opt_state,
              model_params=None, best_loss=float('inf'), train_losses=None, test_losses=None):
    ...
    best_loss_test = best_loss

    print("Epoch, Time per Epoch, Average Loss for train dataset, Average Loss for test dataset")
    # Training loop
    step = 0
    for epoch in range(loss_fn.model_data["num_epoch"]):
        ...
        for batch in train_loader:
            ...
            avg_loss_train += loss_val * len(x)
            # print("training error = ", lossVal)
            step += 1

        avg_loss_train /= sum(len(batch[0]) for batch in train_loader)
        ...
        if avg_loss_train < best_loss:
            ...
            best_model_params = model_params
            print(f"New best train loss: {best_loss}")
        ...
        for batch in test_loader:
            ...
            avg_loss_test += loss_val * len(x)

        avg_loss_test /= sum(len(batch[0]) for batch in test_loader)
        test_losses.append(avg_loss_test)

        if avg_loss_test < best_loss_test:
            best_loss_test = avg_loss_test
            # best_model_params = model_params
            print(f"New best test loss: {best_loss_test}")

        t2 = time.time()
        if epoch % 1 == 0:
            print(epoch, t2 - t1, avg_loss_train, avg_loss_test)
