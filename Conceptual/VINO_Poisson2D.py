...
Variational Physics Informed Neural Operator example for Poisson 2D problem
Energy
Problem statement:
...
Hyperparameters:
s = 2 ** 6,     m = 24,     lr = 0.001,     batch = 100
s = 2 ** 7,     m = 20,     lr = 0.001,     batch = 50
s = 2 ** 8,     m = 28,     lr = 0.001,     batch = 50
s = 2 ** 9,     m = 32,     lr = 0.001,     batch = 25
"""
import os
...
from utils.solvers import Poisson2D_solve, Poisson2D_itsolve, Poisson2D_pyagm_solve, Poisson2D_petsc_solve
from utils.postprocessing import (plot_pred2, plot_loss_trend, plot_two_sets_trajectories,
                                  time_comparison, timing_boxplot, timing_boxplot_multi, timing_violin_multi)
from utils.fno_2d import FNO2d
from utils.fno_utils import count_params, LpLoss, train_vino_poisson
...
#  Parse arguments
################################################################
parser = argparse.ArgumentParser(description="Poisson 2D Solver with VINO")

parser.add_argument("--s", type=int, default=2 ** 6, help="Grid resolution")
parser.add_argument("--modes", type=int, default=24, help="Fourier modes")
parser.add_argument("--batch_size", type=int, default=100, help="Batch size")
parser.add_argument("--learning_rate", type=float, default=0.001, help="Learning rate")

parser.add_argument("--nTrain", type=int, default=1000, help="Number of training samples")
parser.add_argument("--nTest", type=int, default=2000, help="Number of test samples")
parser.add_argument("--L", type=float, default=1.0, help="Domain length")
parser.add_argument("--scaling", type=float, default=100.0, help="Scaling factor")
parser.add_argument("--length_scale_train", type=float, default=0.1, help="Length scale for training data")
parser.add_argument("--length_scale_test", type=float, default=0.1, help="Length scale for test data")
...
parser.add_argument("--use_data", type=bool, default=True, help="Whether to use dataset or regenerate")
parser.add_argument("--load_model", type=bool, default=True, help="Load pretrained model")
parser.add_argument("--training", type=bool, default=False, help="Enable/disable training")
...
length_scale_train = args.length_scale_train
length_scale_test = args.length_scale_test
s = args.s
...
dataset_filename = os.path.join(dataset_dir, f"{dataset_name}nTrain{nTrain}_nTest{nTest}_s{s}.pt")
...
if os.path.exists(dataset_filename):
    ...
    F_train = torch.from_numpy(generate_inputs(nTrain, s, scaling=scaling, length_scale=length_scale_train)).float()
    ...
    F_test = torch.from_numpy(generate_inputs(nTest, s, scaling=scaling, length_scale=length_scale_test)).float()
...
model_name = (
    f"VINO_Poisson2D_nTrain{nTrain}_nTest{nTest}_S{s}_batch{batch_size}"
...
LossFunction = LpLoss(d=1, size_average=False)
...
if training:
    model, train_l2, test_l2 = train_vino_poisson(model, train_loader, test_loader, LossFunction, optimizer,
...
plot_loss_trend(losses, labels,  ylims=((-44.5, -41), (0, 2.0)))
...
with torch.no_grad():
    for x, y in test_loader:
        ...
        pred[index] = out
        test_l2 = LossFunction(out.view(1, -1), y.view(1, -1)).item()
...
u_pred = pred[index, :]
plot_pred2(x_plot_grid, y_plot_grid, u_exact, u_pred, 'GRF', 'Ex1')
print("################################################################")
...
pred = pred.numpy()
errs = error_calculator(y_test2, pred)
print("Average error for VINO prediction =", statistics.mean(errs))
...
solver_name = 'CG'
fig_size = (6, 2.5)
u_cg, time_cg, tr_cg = Poisson2D_itsolve(x_test2, rtol=1e-5)
u_nows_cg, time_nows_cg, tr_nows_cg = Poisson2D_itsolve(x_test2, u0=pred, rtol=1e-5)

time_comparison(time_cg, tr_cg, time_nows_cg, tr_nows_cg, solver_name)
print_errors(u_cg, u_nows_cg, y_test2)
plot_two_sets_trajectories(tr_nows_cg, tr_cg, label1='CG-NOWS', label2='CG', fig_size=fig_size)
# timings_nows, timings_cg = timing_boxplot(tr_nows_cg, tr_cg, label1="CG-NOWS", label2="CG", tol=1e-3)
# timings_nows, timings_cg, perc_savings = timing_boxplot_multi(tr_nows_cg, tr_cg, label1="CG-NOWS", label2="CG")
_, _, _ = timing_violin_multi(tr_nows_cg, tr_cg, label1="CG-NOWS", label2="CG", fig_size=fig_size)
# plot_residual_vs_iteration(tr_cg, tr_nows_cg, solver_time_cg, solver_time_nows_cg)
...
u_pyAMG, time_pyAMG, tr_pyAMG = Poisson2D_pyagm_solve(x_test2, rtol=1e-5)
u_nows_pyAMG, time_nows_pyAMG, tr_nows_pyAMG = Poisson2D_pyagm_solve(x_test2, u0=pred, rtol=1e-5)

time_comparison(time_pyAMG, tr_pyAMG, time_nows_pyAMG, tr_nows_pyAMG, solver_name)
print_errors(u_pyAMG, u_nows_pyAMG, y_test2)
...
u_PETSc, time_PETSc, tr_PETSc = Poisson2D_petsc_solve(x_test2, rtol=1e-6)
u_nows_PETSc, time_nows_PETSc, tr_nows_PETSc = Poisson2D_petsc_solve(x_test2, u0=pred, rtol=1e-6)

time_comparison(time_PETSc, tr_PETSc, time_nows_PETSc, tr_nows_PETSc, solver_name)
print_errors(u_PETSc, u_nows_PETSc, y_test2)
